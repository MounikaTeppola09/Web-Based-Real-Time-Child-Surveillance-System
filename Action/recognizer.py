...
def framewise_recognize(pose, pretrained_model, fn, fno):
    ...
    if bboxes:
        ...
        for d in trk_result:
            ...
            if joints_norm_per_frame.size > 0:
                ...
                cv.putText(frame, init_label, (xmin + 10, ymin - 15), cv.FONT_HERSHEY_SIMPLEX, 1, trk_clr, 1)
                if init_label == 'fall_down':
                    cv.putText(frame, 'ALERT!', (10, 100),cv.FONT_HERSHEY_DUPLEX,1,(0, 0, 255), 1)
                    cv.putText(frame, 'Child is falling! Take immediate action', (10, 130),cv.FONT_HERSHEY_DUPLEX,1,(0, 0, 255), 1)

            cv.rectangle(frame, (xmin - 10, ymin - 30), (xmax + 10, ymax), trk_clr, 1)
            print('detected event:',init_label)
